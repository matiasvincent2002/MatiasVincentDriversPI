...
const getDriversName = async (req, res) => {
  try {
    ...
    const matchingDriversFromDB = dbDrivers.filter(driver =>
      driver.firstName?.toLowerCase()?.includes(queryWord) || // Busca en el campo "forename"
      driver.lastName?.toLowerCase()?.includes(queryWord) // Busca en el campo "surname"
    );
    ...
    const matchingDrivers = matchingDriversFromAPI.concat(matchingDriversFromDB).map(driver => ({
      id: driver.id,
      name: !driver.name.forename?driver.firstName+ ' ' +driver.lastName:driver.name.forename + ' ' + driver.name.surname,
